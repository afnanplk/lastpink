...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*pinky Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*pinky Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*pinky  Working as a Chatbot with the name eva! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using pinky 💌*\n    *- Eva*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```Type``` *.update now* ```to Update Pinky.```\n\n' + degisiklikler + '```', MessageType.text
                    ); 
                }
            }
        }
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Pinky Working as Private! 🐺*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using pinky 🥰*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```Type``` *.update now* ```For The Update Pinky.```\n\n' + degisiklikler + '```', MessageType.text
                    ); 
                }
            }
        }
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [PINKY] --*' + 
                                    '\n*Pinkbot an error has occurred!*'+
                                    '\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n_You can write to our Telegram group for help._' +
                                    '\n_Also you can join our support group:_ https://chat.whatsapp.com/IlmB0J0SilOIJRq1CLotXT' +
